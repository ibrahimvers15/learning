...
import type { SubjectUI, SubjectKey } from './types';
...
const Footer: React.FC = () => (
    <footer className="w-full p-4 bg-gray-800 text-white text-center mt-auto">
        <p>&copy; 2024 Built by Muhammad Ibrahim</p>
    </footer>
);
...
const HomeScreen: React.FC<HomeScreenProps> = ({ onSubjectSelect }) => {
    ...
    const subjectsForGrade = courseData[selectedGrade] || {};
    const availableSubjects = Object.keys(subjectsForGrade).filter(key => SUBJECTS[key]);
        ...
        <div>
            ...
            <div className="space-y-4">
                {chapters.map((chapter, index) => (
                    ...
                    >
                        <span className="text-lg font-bold text-gray-700">{chapter.title}</span>
...
const LessonScreen: React.FC<LessonScreenProps> = ({ grade, subject, chapterTitle, onBack }) => {
    const [view, setView] = useState<'lesson' | 'quiz'>('lesson');
    
    const chapter: Chapter | undefined = courseData[grade]?.[subject.key]?.chapters.find(c => c.title === chapterTitle);
    ...
    if (!chapter) {
        return (
            <div>
                ...
                <Card>
                    <p className="text-center text-red-500 font-bold">Oops! We couldn't find this lesson.</p>
                </Card>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div className="text-center mb-6">
                <h2 className="text-4xl font-black text-gray-800">{chapter.title}</h2>
            ...
            <Card className="min-h-[300px]">
                {view === 'lesson' && (
                    <div>
                         {chapter.image && (
                            <div className="my-4 p-2 border rounded-lg bg-gray-100 shadow-inner">
                                <img src={chapter.image} alt={chapter.title} className="w-full h-auto rounded-md object-contain max-h-80" />
                            </div>
                        )}
                        <p className="text-gray-700 text-lg whitespace-pre-wrap leading-relaxed">{chapter.lesson}</p>
                    </div>
                )}
                {view === 'quiz' && (
...
const QuizComponent: React.FC<QuizComponentProps> = ({ questions }) => {
  ...
  if (!questions || questions.length === 0) {
    return <p className="text-center text-gray-600">No quiz available for this chapter yet.</p>;
  }
...
const App: React.FC = () => {
    ...
    const subject = useMemo((): (SubjectUI & { key: SubjectKey }) | null => {
        if (!subjectKey) return null;
        const subjectUI = SUBJECTS[subjectKey];
        if (!subjectUI) return null;
        return { ...subjectUI, key: subjectKey };
    }, [subjectKey]);
        ...
        switch (view) {
            ...
            case 'lesson':
                if (subject && chapterTitle) {
                    return <LessonScreen grade={grade} subject={subject} chapterTitle={chapterTitle} onBack={backToSubject} />;
                }
                return null;
            case 'home':
            default:
